...
- Replace the three files of the [preview folder](/preview) with an example animation of your benchmark [recorded from Webots](https://cyberbotics.com/doc/guide/web-animation). Keep the same names for the files: animation.json, scene.x3d and thumbnail.jpg.
...
You can now submit your benchmark to [webots.cloud](https://webots.cloud/benchmark) to share it with other people. On the website, in the "Benchmark" tab, click on "Add a new benchmark" and enter the URL to your .wbt world file located in the [worlds folder](./worlds/).
...
### 8. Sharing the Benchmark

You can share the [webots.cloud](https://webots.cloud) link which allows people to try the benchmark online and see the leaderboard or directly the repository link to get the registration instructions. A link to the repository will be also present on the [webots.cloud](https://webots.cloud/benchmark) site.

### 9. Removing the Benchmark from webots.cloud
...
- On [webots.cloud](https://webots.cloud/benchmark), look for your benchmark and click on the synchronization icon in the "Updated" column.
- After a moment there should be message confirming that the benchmark was deleted from the database.
